...
export class AuditService {
  ...
  static async getLastAccess(cardId: string): Promise<AccessHistory> {
    const response = await fetch(`${API_BASE}/api/audit/last-access/${encodeURIComponent(cardId)}`);
    if (!response.ok) {
  ...
  static async getLastSuccessfulAccess(cardId: string): Promise<AccessHistory> {
    const response = await fetch(`${API_BASE}/api/audit/last-successful-access/${encodeURIComponent(cardId)}`);
    if (!response.ok) {
      if (response.status === 404) {
  ...
  static async getEventStatistics(): Promise<AuditStatistics> {
    const response = await fetch(`${API_BASE}/api/audit/statistics/events`);
    if (!response.ok) {
  ...
  static async getAccessStatistics(): Promise<AccessStatistics> {
    const response = await fetch(`${API_BASE}/api/audit/statistics/access`);
    if (!response.ok) {
  ...
  static async findSuspiciousActivity(maxFailures: number = 5): Promise<AuditEvent[]> {
    const response = await fetch(`${API_BASE}/api/audit/suspicious-activity?maxFailures=${maxFailures}`);
    if (!response.ok) {
  ...
  static async findFrequentFailures(maxFailures: number = 3): Promise<FrequentFailure[]> {
    const response = await fetch(`${API_BASE}/api/audit/frequent-failures?maxFailures=${maxFailures}`);
    if (!response.ok) {
  ...
  static async findSuspiciousIPs(maxFailures: number = 5): Promise<SuspiciousIP[]> {
    const response = await fetch(`${API_BASE}/api/audit/suspicious-ips?maxFailures=${maxFailures}`);
    if (!response.ok) {
  ...
  static async getAverageResponseTimeByReader(): Promise<ReaderResponseTime[]> {
    const response = await fetch(`${API_BASE}/api/audit/response-times`);
    if (!response.ok) {
  ...
  static async getEventCount(start: string, end: string): Promise<EventCount> {
    const response = await fetch(`${API_BASE}/api/audit/count/events?start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`);
    if (!response.ok) {
  ...
  static async getAccessCount(start: string, end: string): Promise<AccessCount> {
    const response = await fetch(`${API_BASE}/api/audit/count/access?start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`);
    if (!response.ok) {
  ...
  static async cleanupOldRecords(retentionDays: number = 90): Promise<{ status: string; message: string }> {
    const response = await fetch(`${API_BASE}/api/audit/cleanup?retentionDays=${retentionDays}`, {
      method: 'POST'
    });
    if (!response.ok) {
  ...
  static async exportEvents(filters: AuditFilters = {}): Promise<Blob> {
    ...
    const response = await fetch(`${API_BASE}/api/audit/export/events?${params}`);
    if (!response.ok) {
  ...
  static async exportAccessHistory(filters: AuditFilters = {}): Promise<Blob> {
    ...
    const response = await fetch(`${API_BASE}/api/audit/export/access-history?${params}`);
    if (!response.ok) {
